...
export default function SetupProfilePage() {
  ...
  const handleCompleteSetup = async () => {
    ...
    try {
      // createUserProfile will create or update (auth details) the profile
      // and return the GameUser object.
      const gameProfile = await createUserProfile(firebaseUser);
      
      if (!gameProfile) {
        // This case should ideally be caught by an error in createUserProfile
        throw new Error("Profile could not be created or retrieved. Please try again.");
      }

      toast({
        title: "Profile Setup Complete!",
        description: "Welcome to Tactical Echo, Commander!",
        variant: "default",
      ...
      setError(e.message || "An unexpected error occurred during profile setup. Please try again.");
      toast({
        variant: "destructive",
        title: "Profile Setup Failed",
        description: e.message || "Could not complete profile setup. Please try again.",
      });
    } finally {
